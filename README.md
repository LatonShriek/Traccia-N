...
gli effetti.** Compare come nono esercizio nel selettore ("Compiti a
obiettivi"), con un selettore per scegliere uno dei 10 scenari sotto.
Schermata dedicata con elenco elementi, pulsante "Completa" per ciascuno,
contatore di violazioni in tempo reale, conto alla rovescia se lo scenario
ha un budget di tempo (con fine automatica allo scadere), risultati a fine
sessione, colonne dedicate nel CSV. Testato end-to-end (dipendenze
rispettate/violate, punteggio, salvataggio) prima della consegna.
...
**Come funziona in pratica**: fase di studio (il testo o la lista
compaiono a schermo, con un pulsante "🔊 Ascolta" per la sintesi vocale),
poi il paziente preme "Pronto — passa al richiamo" quando si sente
pronto. Nella fase di richiamo il paziente racconta/richiama a voce
liberamente, e **l'operatore** — non il paziente — spunta su una
checklist a schermo quali idee-chiave (per i brani) o quali parole (per
le liste) sono state effettivamente recuperate; non serve la stessa
formulazione esatta del testo, conta il concetto. Il punteggio (%
recuperato) si calcola da solo. Stesso principio già in uso con
`respMode='operatore'` negli altri esercizi: il richiamo libero è un
giudizio qualitativo, automatizzarlo con riconoscimento vocale
rischierebbe di introdurre errori di misura proprio nel dato che si vuole
raccogliere, per una popolazione che può avere difficoltà di eloquio o di
